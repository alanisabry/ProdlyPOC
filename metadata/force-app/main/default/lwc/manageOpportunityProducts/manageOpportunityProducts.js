...
import { getOpportunityData, getPicklistValues, saveProducts, searchProducts, getProductDependancies } from 'c/opportunityService';
...
export default class ManageOpportunityProducts extends FwElement {
    @api recordId;
    loading = true;
    loadingCart = false;
    ...
    init() {
        ...
        super.callAsync(getOpportunityData, { recordId: this.recordId }).then(opportunityDataJson => {
            ...
            for (let index = 0; index < this.shoppingCart.length; index++) {
                ...
                lineItem.hasAddOn = lineItem.form.Product2.of_Additional_limit_products__c > 0? true:false;
            ...
            //Disable Part of Package for Package checkbox for opps other than New business 
            if(!this.record.Type.includes('New Business')){
                this.disablePartOfPackageCheckbox = true;
            }
            ...
            if(this.record.Account.Company_Size_Grouped__c == 'Large'&& this.record.Platform__c == 'Bynder'&& this.record.Type == 'New Business-Enterprise Cloud'){
        ...
        setTimeout(() => {
            ...
            if (this.searchQuery && this.searchQuery.timestamp === timestamp) {
                ...
                super.callAsync(searchProducts, { recordId: this.recordId, query: query }).then(resultsJson => {
                    ...
                    console.log('Pricing Model:'+this.availableProducts[index].Product2.Pricing_Model__c);
...
//Executed when 'Part of Min Commit?' field is changed
    ...
    onQuantityChange(event){
        const index = event.target.dataset.index;
        if(this.shoppingCart[index].pricingType == 'Special Pricing'){
    ...
    selectProductClick(event) {
        ...
        this.shoppingCart.push({
            ...
            ConvertedDefaultPriceARR: (this.convertedDefaultPriceARR!=null?this.convertedDefaultPriceARR:product.ConvertedUnitPriceARR),
            form: {
                Quantity:   1.00,
                UnitPrice:  product.UnitPrice,
                Discount:   0.00,
                ...
                Description: null,
                Default_Price_ARR__c: (this.defaultPriceARR!=null?this.defaultPriceARR:product.List_Price_ARR_One_off__c),
                Sales_Price_ARR__c: (this.defaultPriceARR!=null?this.defaultPriceARR:product.List_Price_ARR_One_off__c),
                ...
                Part_of_a_package__c: product.Product2.Pricing_Availability__c == 'Enterprise'?true:false
        ...
        super.callAsync(getProductDependancies, { parentProductID: product.Product2Id, oppCurrency: this.record.CurrencyIsoCode, pricebookID: this.record.Pricebook2Id  }).then(depProductJson => {
            ...
            console.log("Result:" +JSON.parse(JSON.stringify(depProductJson)));
            const dependentProductData = JSON.parse(depProductJson);
            console.log("Result2:" +JSON.parse(depProductJson));
            
            //const dependentProductData = JSON.parse(JSON.stringify(reqJson));
                ...
                for (let index = 0; index < this.requiredProducts.length; index++) {
                    ...
                    this.shoppingCart.push({
                        ...
                        form: {
                            ...
                            Part_of_a_package__c: reqProduct.Product2.Pricing_Availability__c == 'Enterprise'?true:false                       
                        }
                    });
            }
        }     
            ...
            const addOn = result[index];
                this.shoppingCart.push({
                    ...
                    form: {
                        ...
                        Part_of_a_package__c: addOn.form.Part_of_a_package__c
                    }
                });
            }
            this.loadingCart = false;
